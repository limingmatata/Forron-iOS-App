...
- (void)viewDidLoad
{
    ...
    NSMutableURLRequest *postRequest = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:@"http://forron.com/web_service/search_user.php"]];
    ...
    if (requestFlag == 1) {
        ...
        if ([result isEqualToString:@"success"]) {
            ...
            NSDictionary *data = [NSDictionary dictionaryWithObjectsAndKeys:
                                  [NSString stringWithFormat:@"%@ has sent you a friend request", username], @"alert",
                                  username, @"username",@"contact_request" ,@"type"
                                  ,requestIDs, @"requestids",userid,@"userid",
                                  nil];
            ...
            [self showAlert:@"Request sent successfully"];
            requestFlag = 1;
            ...
            [self showAlert:@"Your request failed. please check internet connection."];
        }
    }
}
...
-(IBAction)sendRequest:(id)sender
{
    ...
    {
        requestIDs = [NSMutableString stringWithFormat:@"%@,%@", [searchfriendIdArray objectAtIndex:selectionIndex.row],requestIDs];
    }
    requestIDs = [requestIDs substringToIndex:[requestIDs length] - 1];
    requestFlag = 2;
    NSString *userid = [[NSUserDefaults standardUserDefaults] objectForKey:@"userid"];
    NSString *bodyData = [NSString stringWithFormat:@"user_id=%@&request_ids=%@",userid ,requestIDs];
    NSMutableURLRequest *postRequest = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:@"http://forron.com/web_service/send_contact_request.php"]];
    [postRequest setValue:@"application/x-www-form-urlencoded" forHTTPHeaderField:@"Content-Type"];
    [postRequest setHTTPMethod:@"POST"];
    [postRequest setHTTPBody:[NSData dataWithBytes:[bodyData UTF8String] length:strlen([bodyData UTF8String])]];
    NSURLConnection *conn = [[NSURLConnection alloc] initWithRequest:postRequest delegate:self];
